...
from .layer import IPALayer, dispatch_default
...
class IPAModel(BaseTuner):
    ...
    Args:
        ...
        config ([`IPAConfig`]): The configuration of the IPA model.
        adapter_name (`str`): The name of the adapter, defaults to `"default"`.
        low_cpu_mem_usage (`bool`, `optional`, defaults to `False`):
            Create empty adapter weights on meta device. Useful to speed up the loading process.

    Returns:
        `torch.nn.Module`: The IPA model.

    Example:

        ```py
        >>> from transformers import AutoModelForSeq2SeqLM
        >>> from peft import IPAModel, IPAConfig
        ...
        >>> ipa_model = IPAModel(model, config, "default")
        ```

        ```py
        >>> import torch
        >>> import transformers
        >>> from peft import IPAConfig, PeftModel, get_peft_model, prepare_model_for_kbit_training

        >>> rank = ...
        >>> target_modules = ["q_proj", "k_proj", "v_proj", "out_proj", "fc_in", "fc_out", "wte"]
        >>> config = IPAConfig(
        ...     r=4, ipa_alpha=16, target_modules=target_modules, ipa_dropout=0.1, bias="none", task_type="CAUSAL_LM"
        ... )
        ...
        >>> ipa_model = get_peft_model(model, config)
        ```

    **Attributes**:
        - **model** ([`~transformers.PreTrainedModel`]) -- The model to be adapted.
        - **peft_config** ([`IPAConfig`]): The configuration of the IPA model.
    """
    ...
    def _check_new_adapter_config(self, config: IPAConfig) -> None:
        """
    ...
    @staticmethod
    def _check_target_module_exists(ipa_config, key):
        return check_target_module_exists(ipa_config, key)

    def _prepare_model(self, peft_config: IPAConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        pattern_keys = list(chain(ipa_config.rank_pattern.keys(), ipa_config.alpha_pattern.keys()))
        target_name_key = next(filter(lambda key: re.match(rf".*\.{key}$", current_key), pattern_keys), current_key)
        r = ipa_config.rank_pattern.get(target_name_key, ipa_config.r)

        kwargs = {
            "r": r,
            "scaling": ipa_config.scaling,
            "ipa_mode": ipa_config.ipa_mode,
            "fan_in_fan_out": ipa_config.fan_in_fan_out,
            "use_dora": ipa_config.use_dora,
            "ephemeral_gpu_offload": ipa_config.runtime_config.ephemeral_gpu_offload,
            "loaded_in_8bit": getattr(self.model, "is_loaded_in_8bit", False),
            "loaded_in_4bit": getattr(self.model, "is_loaded_in_4bit", False),
        }

        if isinstance(target, IPALayer):
            target.update_layer(
                adapter_name,
                r,
                ipa_dropout=ipa_config.ipa_dropout,
                scaling=ipa_config.scaling,
                use_dora=ipa_config.use_dora,
            )
        else:
            new_module = self._create_new_module(ipa_config, adapter_name, target, **kwargs)
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, IPALayer) and hasattr(m, "bias") and m.bias is not None:
                        m.bias.requires_grad = True
            else:
                raise NotImplementedError(f"Requested bias: {bias}, is not implemented.")

    @staticmethod
    def _create_new_module(ipa_config, adapter_name, target, **kwargs):
        # Collect dispatcher functions to decide what backend to use for the replaced LoRA layer. The order matters,
        # because the first match is always used. Therefore, the default layers should be checked last.
        dispatchers = []

        if ipa_config._custom_modules:
            ...
            def dynamic_dispatch_func(target, adapter_name, ipa_config, **kwargs):
                new_module = None
                ...
                for key, custom_cls in ipa_config._custom_modules.items():
        ...
        for dispatcher in dispatchers:
            new_module = dispatcher(target, adapter_name, ipa_config=ipa_config, **kwargs)
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        ...     if ...:  # some check on name (ex. if 'ipa' in name)
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for module in self.modules():
            if isinstance(module, IPALayer) or isinstance(module, ModulesToSaveWrapper):
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, IPALayer):
                if adapter_name in target.ipa_A:
                    target_ipa_A = target.ipa_A[adapter_name].weight
                    target_ipa_B = target.ipa_B[adapter_name].weight
                elif adapter_name in target.ipa_embedding_A:
                    target_ipa_A = target.ipa_embedding_A[adapter_name]
                    target_ipa_B = target.ipa_embedding_B[adapter_name]
                else:
                    continue

                target_ipa_A.data = target_ipa_A.data * 0.0
                target_ipa_B.data = target_ipa_B.data * 0.0
                if combination_type == "cat":
                    ipas_A, ipas_B = [], []
                    for adapter, weight in zip(adapters, weights):
                        if adapter in target.ipa_A:
                            current_adapter_ipa_A = target.ipa_A[adapter].weight
                            current_adapter_ipa_B = target.ipa_B[adapter].weight
                        elif adapter in target.ipa_embedding_A:
                            current_adapter_ipa_A = target.ipa_embedding_A[adapter]
                            current_adapter_ipa_B = target.ipa_embedding_B[adapter]
                        else:
                            continue
                        ipas_A.append(current_adapter_ipa_A.data * weight * target.scaling[adapter])
                        ipas_B.append(current_adapter_ipa_B.data)

                    if len(ipas_A) == 0:
                        raise ValueError("No matching LoRAs found. Please raise an issue on GitHub.")
                    ipas_A = torch.cat(ipas_A, dim=0)
                    ipas_B = torch.cat(ipas_B, dim=1)
                    target_ipa_A.data[: ipas_A.shape[0], :] = ipas_A
                    target_ipa_B.data[:, : ipas_B.shape[1]] = ipas_B
                    ...
                    target_ipa_A.data, target_ipa_B.data = self._svd_generalized_task_arithmetic_weighted_adapter(
                        combination_type,
                        adapters,
                        weights,
                        new_rank,
                    ...
                    target_ipa_A.data, target_ipa_B.data = self._generalized_task_arithmetic_weighted_adapter(
    ...
    def _svd_generalized_task_arithmetic_weighted_adapter(
        ...
        valid_adapters = []
        valid_weights = []
        is_embedding = any(adapter in target.ipa_embedding_A for adapter in adapters)
        for adapter, weight in zip(adapters, weights):
            if adapter in target.ipa_A or adapter in target.ipa_embedding_A:
        ...
        # based on https://github.com/kohya-ss/sd-scripts/blob/main/networks/svd_merge_ipa.py#L114-L131
    ...
    def _generalized_task_arithmetic_weighted_adapter(
        ...
        for adapter, weight in zip(adapters, weights):
            ...
            ipa_A_deltas.append(current_adapter_ipa_A.data)
            ipa_B_deltas.append(current_adapter_ipa_B.data)
        valid_weights = torch.tensor(valid_weights).to(ipa_A_deltas[0].device)
        ipa_deltas = [ipa_A_deltas, ipa_B_deltas]
        dtype = ipa_A_deltas[0].dtype
        for i, task_tensors in enumerate(ipa_deltas):
            if combination_type == "linear":
                ipa_deltas[i] = task_arithmetic(task_tensors, valid_weights)
            elif combination_type == "ties":
                ipa_deltas[i] = ties(task_tensors, valid_weights, density, majority_sign_method)
            elif combination_type == "dare_linear":
                ipa_deltas[i] = dare_linear(task_tensors, valid_weights, density)
            elif combination_type == "dare_ties":
                ipa_deltas[i] = dare_ties(task_tensors, valid_weights, density, majority_sign_method)
            elif combination_type == "magnitude_prune":
                ipa_deltas[i] = magnitude_prune(task_tensors, valid_weights, density)
            else:
                raise ValueError("Invalid combination type")
        ipa_deltas = [delta.to(dtype) for delta in ipa_deltas]
        return ipa_deltas
    ...
    def merge_and_unload(
        ...
        >>> peft_model_id = "smangrul/falcon-40B-int4-peft-ipa-sfttrainer-sample"
    ...
    def unload(self) -> torch.nn.Module:
        """
        Gets back the base model by removing all the ipa modules without merging. This gives back the original base
        model.
        """
