...
@dataclass
class IPAConfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`IPAModel`].

    Args:
        r (`int`):
            IPA attention dimension (the "rank").
    ...
    r: int = field(default=8, metadata={"help": "IPA attention dimension"})
    ...
    bias: Literal["none", "all", "ipa_only"] = field(
        default="none", metadata={"help": "Bias type for IPA. Can be 'none', 'all' or 'ipa_only'"}
    )
    ...
    alpha_pattern: Optional[dict] = field(
        ...
        metadata={
            "help": (
                "The mapping from layer names or regexp expression to alphas which are different from the default alpha specified by `ipa_alpha`. "
    ...
    runtime_config: IPARuntimeConfig = field(
        default_factory=IPARuntimeConfig, metadata={"help": "Runtime configurations"}
    )
    ...
    def __post_init__(self):
        ...
        # be correctly done when using rsipa + rank_pattern/alpha_pattern. We can't really know if the user intends
