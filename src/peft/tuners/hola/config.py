...
@dataclass
class HolaConfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`HolaModel`].

    Args:
        r (`int`):
            Hola attention dimension (the "rank").
        ...
        hola_alpha (`int`):
            The alpha parameter for Hola scaling.
        hola_dropout (`float`):
            The dropout probability for Hola layers.
        fan_in_fan_out (`bool`):
            Set this to True if the layer to replace stores weight like (fan_in, fan_out). For example, gpt-2 uses
            `Conv1D` which stores weights like (fan_in, fan_out) and hence this should be set to `True`.
        bias (`str`):
            Bias type for Hola. Can be 'none', 'all' or 'hola_only'. If 'all' or 'hola_only', the corresponding biases
            will be updated during training. Be aware that this means that, even when disabling the adapters, the model
            will not produce the same output as the base model would have without adaptation.
        use_rshola (`bool`):
            When set to True, uses <a href='https://doi.org/10.48550/arXiv.2312.03732'>Rank-Stabilized Hola</a> which
            sets the adapter scaling factor to `hola_alpha/math.sqrt(r)`, since it was proven to work better.
            Otherwise, it will use the original default value of `hola_alpha/r`.
        modules_to_save (`List[str]`):
            List of modules apart from adapter layers to be set as trainable and saved in the final checkpoint.
        init_hola_weights (`bool` | `Literal["gaussian", "ohola", "pissa", "pissa_niter_[number of iters]", "loftq"]`):
            How to initialize the weights of the adapter layers. Passing True (default) results in the default
            initialization from the reference implementation from Microsoft. Passing 'gaussian' results in Gaussian
            initialization scaled by the Hola rank for linear and layers. Setting the initialization to False leads to
            completely random initialization and is discouraged. Pass `'loftq'` to use LoftQ initialization. Pass
            `'ohola'` to use OHola initialization. Passing `'pissa'` results in the initialization of <a
            href='https://arxiv.org/abs/2404.02948'>Principal Singular values and Singular vectors Adaptation
            (PiSSA)</a>, which converges more rapidly than Hola and ultimately achieves superior performance. Moreover,
            PiSSA reduces the quantization error compared to QHola, leading to further enhancements. Passing
        ...
        alpha_pattern (`dict`):
            ...
            specified by `hola_alpha`.
        megatron_config (`Optional[dict]`):
            The TransformerConfig arguments for Megatron. It is used to create Hola's parallel linear layer. You can
            ...
            parameter when you want to apply Hola to the ColumnParallelLinear and RowParallelLinear layers of megatron.
        megatron_core (`Optional[str]`):
            The core module from Megatron to use, defaults to `"megatron.core"`.
        loftq_config (`Optional[LoftQConfig]`):
            The configuration of LoftQ. If this is not None, then LoftQ will be used to quantize the backbone weights
            and initialize Hola layers. Also pass `init_hola_weights='loftq'`. Note that you should not pass a
            quantized model in this case, as LoftQ will quantize the model itself.
        use_dora (`bool`):
            Enable 'Weight-Decomposed Low-Rank Adaptation' (DoRA). This technique decomposes the updates of the weights
            into two parts, magnitude and direction. Direction is handled by normal Hola, whereas the magnitude is
            handled by a separate learnable parameter. This can improve the performance of Hola especially at low
            ranks. Right now, DoRA only supports linear and Conv2D layers. DoRA introduces a bigger overhead than pure
            Hola, so it is recommended to merge weights for inference. For more information, see
            https://arxiv.org/abs/2402.09353.
        layer_replication (`List[Tuple[int, int]]`):
            Build a new stack of layers by stacking the original model layers according to the ranges specified. This
            allows expanding (or shrinking) the model without duplicating the base model weights. The new layers will
            all have separate Hola adapters attached to them.
        runtime_config (`HolaRuntimeConfig`):
            Runtime configurations (which are not saved or restored).
    """

    r: int = field(default=8, metadata={"help": "Hola attention dimension"})
    target_modules: Optional[Union[list[str], str]] = field(
        default=None,
        metadata={
            "help": (
                "List of module names or regex expression of the module names to replace with Hola."
    ...
    hola_alpha: int = field(default=8, metadata={"help": "Hola alpha"})
    hola_code_size: int =  field(default=64, metadata={"help": ""})
    hola_dropout: float = field(default=0.0, metadata={"help": "Hola dropout"})
    fan_in_fan_out: bool = field(
        default=False,
        metadata={"help": "Set this to True if the layer to replace stores weight like (fan_in, fan_out)"},
    )
    bias: Literal["none", "all", "hola_only"] = field(
        default="none", metadata={"help": "Bias type for Hola. Can be 'none', 'all' or 'hola_only'"}
    )
    modules_to_save: Optional[list[str]] = field(
        default=None,
        metadata={
            "help": "List of modules apart from Hola layers to be set as trainable and saved in the final checkpoint. "
            "For example, in Sequence Classification or Token Classification tasks, "
            "the final layer `classifier/score` are randomly initialized and as such need to be trainable and saved."
        },
    )
    init_hola_weights: Union[bool, Literal["gaussian"]] = field(
        default=True,
        metadata={
            "help": (
                "How to initialize the weights of the Hola layers. Passing `'True'` (default) results in the default "
                "initialization from the reference implementation from Microsoft. Passing `'gaussian'` results "
                "in Gaussian initialization scaled by the Hola rank for linear and layers. Setting the initialization "
    ...
    alpha_pattern: Optional[dict] = field(
        ...
        metadata={
            "help": (
                "The mapping from layer names or regexp expression to alphas which are different from the default alpha specified by `hola_alpha`. "
    ...
    megatron_config: Optional[dict] = field(
        ...
        metadata={
            "help": (
                "The TransformerConfig from Megatron. It is used to create Hola's parallel linear layer."
                "You can get it like this, `core_transformer_config_from_args(get_args())`, "
                "these two functions being from Megatron."
                "You need to specify this parameter when you want to apply Hola to the ColumnParallelLinear and "
    ...
    megatron_core: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The core module from Megatron, it is used to create Hola's parallel linear layer. "
    ...
    layer_replication: Optional[list[tuple[int, int]]] = field(
        ...
        metadata={
            "help": (
                "This enables using Hola to effectively expand a transformer model to a larger size by repeating some layers. "
                ...
                "remain fixed during finetuning but each layer has a separate Hola adapter so the layers can be specialed via "
    ...
    runtime_config: HolaRuntimeConfig = field(
        default_factory=HolaRuntimeConfig, metadata={"help": "Runtime configurations"}
    )
    ...
    def _register_custom_module(self, mapping: dict[type[nn.Mmodule], type[nn.Module]]) -> None:
        """
        Experimental API to support providing custom Hola layers.
        ...
        To register custom Hola module types, call this method with a `mapping` argument that is a dict that maps from
        the target layer type to the custom Hola layer type. The dict can contain multiple items if you wish to target
        multiple layer types. The target layer type can be any nn.Module that we currently don't support in PEFT,
        whether that is an official PyTorch layer type or a custom layer type. The custom Hola module class has to be
        implemented by the user and follow the PEFT conventions for Hola layers.
