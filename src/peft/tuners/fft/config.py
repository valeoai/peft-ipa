...
class FFTConfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`FFTModel`].

    Args:
        r (`int`):
            FFT attention dimension (the "rank").
        ...
        hola_alpha (`int`):
            The alpha parameter for FFT scaling.
        hola_dropout (`float`):
            The dropout probability for FFT layers.
        fan_in_fan_out (`bool`):
            Set this to True if the layer to replace stores weight like (fan_in, fan_out). For example, gpt-2 uses
            `Conv1D` which stores weights like (fan_in, fan_out) and hence this should be set to `True`.
        bias (`str`):
            Bias type for FFT. Can be 'none', 'all' or 'hola_only'. If 'all' or 'hola_only', the corresponding biases
            will be updated during training. Be aware that this means that, even when disabling the adapters, the model
            will not produce the same output as the base model would have without adaptation.
        use_rshola (`bool`):
            When set to True, uses <a href='https://doi.org/10.48550/arXiv.2312.03732'>Rank-Stabilized FFT</a> which
        ...
        init_hola_weights (`bool` | `Literal["gaussian", "ohola", "pissa", "pissa_niter_[number of iters]", "loftq"]`):
            ...
            initialization scaled by the FFT rank for linear and layers. Setting the initialization to False leads to
            completely random initialization and is discouraged. Pass `'loftq'` to use LoftQ initialization. Pass
            `'ohola'` to use OFFT initialization. Passing `'pissa'` results in the initialization of <a
            href='https://arxiv.org/abs/2404.02948'>Principal Singular values and Singular vectors Adaptation
            (PiSSA)</a>, which converges more rapidly than FFT and ultimately achieves superior performance. Moreover,
            PiSSA reduces the quantization error compared to QFFT, leading to further enhancements. Passing
        ...
        megatron_config (`Optional[dict]`):
            The TransformerConfig arguments for Megatron. It is used to create FFT's parallel linear layer. You can
            ...
            parameter when you want to apply FFT to the ColumnParallelLinear and RowParallelLinear layers of megatron.
        megatron_core (`Optional[str]`):
            The core module from Megatron to use, defaults to `"megatron.core"`.
        loftq_config (`Optional[LoftQConfig]`):
            The configuration of LoftQ. If this is not None, then LoftQ will be used to quantize the backbone weights
            and initialize FFT layers. Also pass `init_hola_weights='loftq'`. Note that you should not pass a
            quantized model in this case, as LoftQ will quantize the model itself.
        use_dora (`bool`):
            Enable 'Weight-Decomposed Low-Rank Adaptation' (DoRA). This technique decomposes the updates of the weights
            into two parts, magnitude and direction. Direction is handled by normal FFT, whereas the magnitude is
            handled by a separate learnable parameter. This can improve the performance of FFT especially at low
            ranks. Right now, DoRA only supports linear and Conv2D layers. DoRA introduces a bigger overhead than pure
            FFT, so it is recommended to merge weights for inference. For more information, see
            https://arxiv.org/abs/2402.09353.
        layer_replication (`List[Tuple[int, int]]`):
            Build a new stack of layers by stacking the original model layers according to the ranges specified. This
            allows expanding (or shrinking) the model without duplicating the base model weights. The new layers will
            all have separate FFT adapters attached to them.
        runtime_config (`FFTRuntimeConfig`):
    ...
    target_modules: Optional[Union[list[str], str]] = field(
        ...
        metadata={
            "help": (
                "List of module names or regex expression of the module names to replace with FFT."
    ...
    bias: Literal["none", "all", "hola_only"] = field(
        default="none", metadata={"help": "Bias type for FFT. Can be 'none', 'all' or 'hola_only'"}
    )
    modules_to_save: Optional[list[str]] = field(
        default=None,
        metadata={
            "help": "List of modules apart from FFT layers to be set as trainable and saved in the final checkpoint. "
            "For example, in Sequence Classification or Token Classification tasks, "
            "the final layer `classifier/score` are randomly initialized and as such need to be trainable and saved."
        },
    )
    init_fft_weights: Union[bool, Literal["gaussian"]] = field(
        default=True,
        metadata={
            "help": (
                "How to initialize the weights of the FFT layers. Passing `'True'` (default) results in the default "
                "initialization from the reference implementation from Microsoft. Passing `'gaussian'` results "
                "in Gaussian initialization scaled by the FFT rank for linear and layers. Setting the initialization "
    ...
    megatron_config: Optional[dict] = field(
        ...
        metadata={
            "help": (
                "The TransformerConfig from Megatron. It is used to create FFT's parallel linear layer."
                "You can get it like this, `core_transformer_config_from_args(get_args())`, "
                "these two functions being from Megatron."
                "You need to specify this parameter when you want to apply FFT to the ColumnParallelLinear and "
    ...
    megatron_core: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The core module from Megatron, it is used to create FFT's parallel linear layer. "
    ...
    layer_replication: Optional[list[tuple[int, int]]] = field(
        ...
        metadata={
            "help": (
                "This enables using FFT to effectively expand a transformer model to a larger size by repeating some layers. "
                ...
                "remain fixed during finetuning but each layer has a separate FFT adapter so the layers can be specialed via "
    ...
    runtime_config: FFTRuntimeConfig = field(
        default_factory=FFTRuntimeConfig, metadata={"help": "Runtime configurations"}
    )
    ...
    def _register_custom_module(self, mapping: dict[type[nn.Mmodule], type[nn.Module]]) -> None:
        """
        Experimental API to support providing custom FFT layers.
        ...
        To register custom FFT module types, call this method with a `mapping` argument that is a dict that maps from
        the target layer type to the custom FFT layer type. The dict can contain multiple items if you wish to target
        multiple layer types. The target layer type can be any nn.Module that we currently don't support in PEFT,
        whether that is an official PyTorch layer type or a custom layer type. The custom FFT module class has to be
        implemented by the user and follow the PEFT conventions for FFT layers.
