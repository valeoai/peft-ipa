...
def get_peft_model_state_dict(
    ...
    elif config.peft_type == PeftType.IPA:
        # to_return = lora_state_dict(model, bias=model.peft_config.bias)
        # adapted from `https://github.com/microsoft/LoRA/blob/main/loralib/utils.py`
        # to be used directly with the state dict which is necessary when using DeepSpeed or FSDP
        bias = config.bias
        if bias == "none":
            to_return = {k: state_dict[k] for k in state_dict if "ipa_" in k}
        elif bias == "all":
            to_return = {k: state_dict[k] for k in state_dict if "ipa_" in k or "bias" in k}
        elif bias == "ipa_only":
            to_return = {}
            for k in state_dict:
                if "ipa_" in k:
                    to_return[k] = state_dict[k]
                    bias_name = k.split("ipa_")[0] + "bias"
                    if bias_name in state_dict:
                        to_return[bias_name] = state_dict[bias_name]
        else:
            raise NotImplementedError
        to_return = {k: v for k, v in to_return.items() if (("ipa_" in k and adapter_name in k) or ("bias" in k))}
    ...
    elif config.peft_type == PeftType.HOLA:
        to_return = {k: state_dict[k] for k in state_dict if "hola_" in k}

    elif config.peft_type == PeftType.FFT:
        to_return = {k: state_dict[k] for k in state_dict if "fft_" in k}
    ...
    elif config.peft_type == PeftType.OURS:
        to_return = {k: state_dict[k] for k in state_dict if "ours_code_" in k}
