...
def main(
    ...
    if args.adapter == "LoRA" or args.adapter == "DoRA" or args.adapter == "IPA":
        ...
        for key, module in key_list:
            target_module_found = any(key.endswith(target_key) for target_key in ["q_proj", "k_proj", "v_proj", "up_proj", "down_proj"])

            if target_module_found:
                print(f"found {key}")
                module.merge(adapter_names=["default"])
...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=["boolq", "piqa", "social_i_qa", "hellaswag", "winogrande", "ARC-Challenge", "ARC-Easy", "openbookqa"],
                        required=True)
    parser.add_argument('--model', choices=['LLaMA-7B', "LLaMA-13B",'LLaMA2-7B','LLaMA3-8B', 'Gemma3-4B'])
    parser.add_argument('--adapter', choices=['LoRA', 'AdapterP', 'AdapterH', 'Parallel', 'DoRA', 'IPA'],
                        required=True)
